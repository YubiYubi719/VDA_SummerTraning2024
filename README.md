...
(1,2)，也就是物品 1 放在上方，2 在下方。那麼，取用 1 的時候不需要能量，而每次取用 2 的能量消耗是 w(1)=1，因為 2 需要取用 f(2)=4 次，所以消耗能量數為 w(1)*f(2)=4。

(2,1)，也就是物品 2 放在 1 的上方。那麼，取用 2 的時候不需要能量，而每次取用 1 的能量消耗是 w(2)=2，因為 1 需要取用 f(1)=3 次，所以消耗能量數為 w(2)*f(1)=6。
